...
_SELF_GLOSSARY = {
    ...
    "RssAtStartBytes": "Resident memory the moment imports finished and before a single request was served. This is the question 'why is the observer the largest process on my small host' made answerable: most of it is the interpreter and the import graph, not anything this product accumulated. Measured on a development machine, 62 MiB before serving anything. Roughly 26 MiB of that is the web framework's import graph, but only about 13 MiB of it is FastAPI ABOVE Starlette — the rest is Starlette, which the product needs either way. A reader planning a reclaim from this figure should use the smaller number.",
