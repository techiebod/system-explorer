...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect subnets\n",
		"token with an empty gen":       "collect subnets:\n",
		"token with a non-integer gen":  "collect subnets:abc\n",
		"token with a signed gen":       "collect subnets:-1\n",
		"token with a plus-signed gen":  "collect subnets:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Subnets:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect subnets:1 subnets:2\n",
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	code, stdout, stderr := runWith(t, "collect pools:11 daemon:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "pools" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for pools, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "pools" {
			t.Fatal("an unsupported decline established nothing and must not commit")
		}
	}
}

// The four collections are served in the order the request line names them, and
// each under its OWN issued generation — the whole of the newest-wins mechanism
// is that echo, and a collector that answered under one generation for all four
// would let a stale batch retire three collections it never read.
func TestEachCollectionCommitsUnderItsOwnIssuedGeneration(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	code, stdout, stderr := runWith(t, "collect subnets:5 daemon:6 reservations:7\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	want := map[string]float64{"subnets": 5, "daemon": 6, "reservations": 7}
	seen := map[string]float64{}
	var order []string
	for _, commit := range ofKind(records, "commit") {
		collection := commit["collection"].(string)
		seen[collection] = commit["generation"].(float64)
		order = append(order, collection)
	}
	for collection, generation := range want {
		if seen[collection] != generation {
			t.Errorf("%s committed under %v, issued %v", collection, seen[collection], generation)
		}
	}
	if strings.Join(order, ",") != "subnets,daemon,reservations" {
		t.Errorf("the request line's order is the batch's order; got %v", order)
	}
}
