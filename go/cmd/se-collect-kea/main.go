// Command se-collect-kea is the DHCP-server collector. It reads one request
// line on standard input and writes to standard output; under socket activation
// systemd supplies both from the accepted connection, and in a terminal a person
// does. Those are the same contract, which is what makes this binary testable
// with a pipe and judged by the same harness that judges the Python reference it
// was ported from.
package main
...
// never travel the stream legally — refusing it at the request is "I could not
// run", not a sanitisation.
...
func run(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "object", "evidence":
		...
		// (DESIGN 18). The name is data — a CIDR or an address carries no
		// whitespace, and one that somehow did would arrive as extra tokens
		// and be refused whole rather than reassembled.
		if len(fields) != 3 {
