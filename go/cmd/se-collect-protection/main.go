// Command se-collect-protection is the protection collector: what the estate
// promises to keep, whether each promise is built, whether it ran, and whether
// anything has ever been restored from it. It reads one request line on
// standard input and writes to standard output; under socket activation
// systemd supplies both from the accepted connection, and in a terminal a
// person does. Those are the same contract, which is what makes this binary
// testable with a pipe and judged by the same harness that judges the Python
// reference it was ported from.
//
// Three documents the host already publishes are read as FILES and nothing
// else — a rendered declaration under /etc, an hourly staleness verdict, and
// one receipt per job per outcome under /var/lib. The repositories those jobs
// write to are deliberately never reached: reading the off-site one needs the
// delete-capable credential that lives with the maintenance identity, and an
// observer holding it would break the estate's own rule that a source cannot
// prune its own history. So every fact here comes from a declaration, a
// verdict or a receipt, and this binary opens no socket and holds no key.
package main
...
const (
	exitOK      = 0
	exitRuntime = 1 // a document this binary needs and could not read
