...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect jobs\n",
		"token with an empty gen":       "collect jobs:\n",
		"token with a non-integer gen":  "collect jobs:abc\n",
		"token with a signed gen":       "collect jobs:-1\n",
		"token with a plus-signed gen":  "collect jobs:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Jobs:5\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect jobs:3\ncollect jobs:4\n", env)
	if code != exitOK {
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	code, stdout, stderr := runWith(t, "collect repositories:11 targets:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "repositories" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for repositories, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "repositories" {
...
func TestTheCollectionOrderIsTheRequestLines(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	_, stdout, _ := runWith(t, "collect jobs:1 destinations:2 targets:3\n", env)
	var order []string
	for _, commit := range ofKind(parseRecords(t, stdout), "commit") {
		order = append(order, commit["collection"].(string))
	}
	want := []string{"jobs", "destinations", "targets"}
	for i := range want {
