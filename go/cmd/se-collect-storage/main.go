// Command se-collect-storage is the ZFS pools collector. It reads one
// request line on standard input and writes to standard output; under socket
// activation systemd supplies both from the accepted connection, and in a
// terminal a person does. Those are the same contract, which is what makes
// this binary testable with a pipe and judged by the same harness that
// judges the Python reference it was ported from.
package main
...
// decline (DESIGN 18) — a collector that ran and found nothing exits zero
// and says so in a record, because a non-zero exit is indistinguishable
// from a crash and both would read as absence.
const (
	exitOK      = 0
...
func run(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "declare":
		...
		// The exact embedded bytes, stable across runs: these are the
		// bytes begin's declaration hash covers, so emitting anything
		// re-serialised here would un-anchor the hash.
	...
	case "object", "evidence":
		...
		// reassembled if whitespace splits it further; dataset and mount
		// names legitimately carry slashes, never spaces.
		if len(fields) != 3 {
	...
	case "lookup":
		...
		return serveLookup(stdout, stderr, src, fields[1], decodeNameToken(fields[2]))
	default:
		// Every token is data, never an option or a command fragment
		// (DESIGN 18) — an unknown verb is refused whole, not guessed at.
		fmt.Fprintf(stderr, "unknown verb %q: this collector serves declare, probe, collect, object, evidence and lookup\n", fields[0])
		return exitRequest
	}
}
...
// everything after the LAST colon, because names in this product
// legitimately contain colons (DESIGN 18). The collator issued it and this
// side only echoes it — in begin's map and on the collection's commit —
// which is the whole of the newest-wins mechanism (DESIGN 19), so a token
// this side cannot parse into an echo is a request that was never run.
func parseCollect(tokens []string) ([]string, map[string]uint64, error) {
	...
	for _, token := range tokens {
		...
		// ParseUint base 10 rejects signs, spaces and prefixes, so
		// "digits only" needs no second check — and a generation too
		// large for uint64 was not issued by any collator.
