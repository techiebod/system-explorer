// Command se-collect-logs is the systemd-journal collector. It reads one
...
func run(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "object", "evidence":
		...
		// (DESIGN 18). A journal cursor carries no whitespace; every token
		// is data, refused whole rather than reassembled.
		if len(fields) != 3 {
