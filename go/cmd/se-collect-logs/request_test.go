...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect journal\n",
		"token with an empty gen":       "collect journal:\n",
		"token with a non-integer gen":  "collect journal:abc\n",
		"token with a signed gen":       "collect journal:-1\n",
		"token with a plus-signed gen":  "collect journal:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Journal:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect journal:1 journal:2\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	dir := stageEntries(t, healthyEntries)
	code, stdout, stderr := runWith(t, "collect journal:3\ncollect journal:4\n",
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	dir := stageEntries(t, healthyEntries)
	code, stdout, stderr := runWith(t, "collect boots:11 journal:12\n",
		map[string]string{"SE_REPLAY_DIR": dir})
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	...
	if len(declines) != 1 || declines[0]["collection"] != "boots" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for boots, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "boots" {
