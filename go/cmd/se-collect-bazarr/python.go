...
// The reference builds this collection's HealthIssues entries as PYTHON TEXT,
// not as JSON: each issue is `f"{subject}: {issue}"` over two raw document
// values, run through the envelope's bounding and scrubbing. That lands in a
// string fact the harness compares character for character, so Python's
// spellings are part of the answer — its str(True) == "True", its "None" for a
// missing member, its repr of a container. This file is that vocabulary, kept
// together so the reasons live in one place rather than scattered through the
// derivation. It is the same file se-collect-network carries, trimmed to what
// one f-string and one bounded reason need.

// ── str() and repr() ────────────────────────────────────────────────────

// pythonStr is str(value) over a decoded JSON document — what the f-string in
// health_facts interpolates. A JSON string interpolates as itself; everything
// else goes through repr, which is why a null subject would read as the
// four-character word None rather than as an empty string.
...
// printable. Reachable only where a health entry puts a container where bazarr
// puts a sentence, which no committed payload does — written out rather than
// approximated so that shape reads the same on both sides.
...
// exactly as written. Only the interpolated issue text depends on this; a
// version fact carries the literal itself, because the harness compares
// parsed values there.
...
// applies by default and health_facts takes as it comes.
const maxReasonLength = 400

// oneLine collapses whitespace and bounds on a word boundary. The rule is
// stated in characters rather than bytes, and a health sentence may carry a
// title in any script, so the count runs over runes.
...
// collector's output depend on the environment of whoever ran it — and this
// collector is itself addressed by SE_BAZARR_API_KEY, so the substitution
// would fire on its own credential and make one payload replay two ways. The
// ruling that parks the environment half is DESIGN's "the residue scrubber may
// not read ambient environment"; the two rules kept are functions of the
// payload alone.
...
// text, not a URL parse, so it fires on any question mark a health sentence
// happens to hold.
