...
// deployment names no bazarr URL fronts no bazarr: the `instance` collection is
...
// host that HAD a bazarr and lost it would otherwise serve its last instance
// row forever, stale and never retired.
//
...
var declineNoInstance = declined{"absent", "no bazarr api on this host"}
