...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageInstance(t, healthyStatus, healthyHealth)}
	code, stdout, stderr := runWith(t, "collect providers:11 instance:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "providers" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for providers, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "providers" {
