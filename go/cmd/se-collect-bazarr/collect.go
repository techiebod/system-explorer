...
// randomise the row's order and route a version member through a float64.
//
// There is no `absent` member and no `names` member, and both are absences by
// construction rather than by omitempty. This collector never says the instance
// genuinely lacks a property — a version it does not carry is a manager this
// bazarr is not wired to, which rule 7 omits — and the row publishes no name
// family at all: bazarr exposes no identifier that survives a rename, the
// reference's summary carries none, and inventing one here would key the
...
func collect(stdout, stderr io.Writer, src source, order []string, generations map[string]uint64) int {
	...
	out.emit(beginRecord{
		...
		// (DESIGN 19); this collector fronts the one bazarr its environment
