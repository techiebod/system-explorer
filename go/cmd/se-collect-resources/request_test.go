...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect workloads\n",
		"token with an empty gen":       "collect workloads:\n",
		"token with a non-integer gen":  "collect workloads:abc\n",
		"token with a signed gen":       "collect workloads:-1\n",
		"token with a plus-signed gen":  "collect workloads:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Workloads:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect workloads:1 workloads:2\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect workloads:3\ncollect workloads:4\n", env)
	if code != exitOK {
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	code, stdout, stderr := runWith(t, "collect zones:11 workloads:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
