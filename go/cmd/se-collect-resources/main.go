// Command se-collect-resources is the workload consumption collector: what
// each cgroup consumed, and what it was stalled on. It reads one request line
// on standard input and writes to standard output; under socket activation
...
//
// Its interface is a TREE rather than a document, which is the one thing that
// makes this port different from the eight before it: the native document is
// the filesystem transcribed — the listings the walk yielded plus the contents
// of every file read (DESIGN's tree-shape ruling, 2026-08-19) — so the replay
// seam here redirects a walk as well as a read, and a replayed run enumerates
// the CAPTURED tree rather than the tree of whichever machine is replaying.
package main
