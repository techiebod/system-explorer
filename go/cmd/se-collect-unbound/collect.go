...
// Facts are raw because their number TOKENS are decided upstream, where the
// document was parsed: a map[string]any here would route every counter
// through a float64 and re-render a figure the payload spelled exactly.
//
// There is no `absent` member and no `names` member, and both are absences by
// construction rather than by omitempty. A record this parse could not lift is
// not a property the daemon was asked about and denied — `stats_noreset`
// carries what this build of unbound reports and nothing declares the rest
// missing — and the row publishes no name family, because the reference's
	...
	for _, collection := range order {
		if collection != collectionDaemon {
			...
			out.emit(declineRecord{
				...
				Reason:     "unsupported",
				Detail:     "this collector serves daemon only",
			})
			continue
		}
		if code := collectDaemon(out, stderr, src, collection, generations[collection], &objects); code != exitOK {
			return code
		}
	}
