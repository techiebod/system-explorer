...
// no control socket configured, or none at the configured path, runs no
// resolver this collector can see — a successful reading that establishes
// something, which is DESIGN 19's own worked example. It must commit zero,
// because a host that HAD unbound and lost it would otherwise serve its last
// daemon row forever, stale and never retired.
//
...
var declineNoSocket = declined{"absent", "no unbound control socket on this host"}
