...
// type and not the language's (DESIGN 19's ruling). `21` and `21.0` are
// different answers under the typed equality the judge applies, and unbound's
// counters are unsigned 64-bit — a query total past 2^53 would lose its last
// digits on a round trip through a float and nothing downstream could tell.
// So the type is decided once, where the line is parsed, and the value is
// re-emitted from the bytes it was read from.
//
