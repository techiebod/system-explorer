...
// (corpus/unbound/healthy): the one row behind an object answer, the two
// control-socket texts behind evidence, the checkable digest, and the
...
func corpusHealthy(t *testing.T) map[string]string {
	t.Helper()
	source := filepath.Join("..", "..", "..", "corpus", "unbound", "healthy", "payloads")
	if _, err := os.Stat(source); err != nil {
		t.Skipf("the unbound healthy payloads are not here: %v", err)
	}
...
func TestObjectServesTheRowTheCollectionPublishes(t *testing.T) {
	code, stdout, stderr := runVerb(t, "object daemon unbound")
	if code != exitOK {
	...
	object := objects[0]
	if object["type"] != "unbound-daemon" || object["name"] != "unbound" {
		t.Fatalf("%+v", object)
	}
	// The row's own facts, unchanged: the object response and the listing
	// must be one answer about one daemon.
	facts := object["facts"].(map[string]any)
	if facts["Version"] == nil || facts["NumQueries"] == nil {
		t.Fatalf("%+v", facts)
	}
...
func TestEvidenceServesBothTextsAndTheDigestChecks(t *testing.T) {
	code, stdout, stderr := runVerb(t, "evidence daemon unbound")
	if code != exitOK {
		t.Fatalf("exit %d: %s", code, stderr)
	}
	...
	// The daemon's own texts, not the row's reading of them.
	var document map[string]string
	if err := json.Unmarshal([]byte(payload), &document); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(document["status"], "version:") ||
		!strings.Contains(document["stats_noreset"], "total.num.queries=") {
		t.Fatalf("both control-socket documents, verbatim: %v", document)
	}
}
...
func TestAVerbDeclineIsDataNotACrash(t *testing.T) {
	...
	code, stdout, _ = runVerb(t, "evidence daemon bind9")
	if code != exitOK {
		t.Fatalf("%d", code)
	}
	records = parseRecords(t, stdout)
...
func TestTheVerbRequestShapeIsExactlyThreeTokens(t *testing.T) {
	for _, request := range []string{"object daemon", "object daemon a b", "evidence daemon"} {
