...
// Facts are raw because their member order and their value types are decided
// upstream, where the document was parsed: a map[string]any here would
// randomise the row's order and route a count of 63195054080 through a
// float64.
//
// There is no `absent` member and no `names` member, and both are absences by
// construction rather than by omitempty. This collector never says the archive
// genuinely lacks a property — a count a paperless is too old to state is a
// member the API never wrote, which rule 7 omits — and the row publishes no
// name family at all: paperless exposes no identifier that survives a rename,
// the object's native name is the constant `paperless`, and inventing one here
// would key the collator on a name no other implementation publishes.
...
func collect(stdout, stderr io.Writer, src source, order []string, generations map[string]uint64) int {
	...
	out.emit(beginRecord{
		...
		BootID:      bootID,
		// Null, and that is a reading of this contract rather than an
		// oversight. `instance` scopes every id beneath it and carries the
		// FLEET-assigned name of one of several instances of an application
		// (DESIGN 19); this collector fronts the one paperless its environment
		// addresses and is told no fleet name, so null — host-native — is the
		// only honest value, and it is what the reference emits.
		Instance:    nil,
		Timens:      src.timens(),
	...
	for _, collection := range order {
		if collection != collectionInstance {
			...
			out.emit(declineRecord{
				...
				Reason:     "unsupported",
				Detail:     "this collector serves instance only",
			})
			continue
		}
		if code := collectInstance(out, stderr, src, collection, generations[collection], &objects); code != exitOK {
			return code
		}
	}
