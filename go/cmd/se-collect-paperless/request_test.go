...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect instance\n",
		"token with an empty gen":       "collect instance:\n",
		"token with a non-integer gen":  "collect instance:abc\n",
		"token with a signed gen":       "collect instance:-1\n",
		"token with a plus-signed gen":  "collect instance:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Instance:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect instance:1 instance:2\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect instance:3\ncollect instance:4\n", env)
	if code != exitOK {
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageInstance(t, healthyStatistics, healthyStatus)}
	code, stdout, stderr := runWith(t, "collect documents:11 instance:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "documents" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for documents, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "documents" {
