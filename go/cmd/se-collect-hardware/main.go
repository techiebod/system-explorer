// Command se-collect-hardware is the physical-inventory collector: what
// lshw/lspci/lsusb print, taken from the kernel's own interfaces instead. It
// reads one request line on standard input and writes to standard output;
// under socket activation systemd supplies both from the accepted connection,
// and in a terminal a person does. Those are the same contract, which is what
// makes this binary testable with a pipe and judged by the same harness that
...
func run(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "object", "evidence":
		...
		// (DESIGN 18). The name is data — a sysfs device name carries no
		// whitespace, and one that somehow did would arrive as extra tokens
		// and be refused whole rather than reassembled into a path.
		if len(fields) != 3 {
