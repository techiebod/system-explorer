...
// Facts are raw because their member order and their number tokens are
// decided upstream, by the document model: a map[string]any here would
// randomise the row's order and re-render a figure the payload spelled.
//
// There is no `absent` member, and that is an absence by construction rather
// than by omitempty: this collection never says a generation genuinely lacks
// a property. A closure that records no release, no configuration revision
// and no build manifest is one that was built without them, which rule 7
// omits rather than declares absent.
...
const collectionGenerations = "generations"
	...
	for _, collection := range order {
		if collection != collectionGenerations {
			...
			out.emit(declineRecord{
				...
				Reason:     "unsupported",
				Detail:     "this collector serves generations only",
			})
			continue
		}
		if code := collectGenerations(out, stderr, src, collection, generations[collection], &objects); code != exitOK {
			return code
		}
	}
