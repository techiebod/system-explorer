...
// construction rather than by omitempty. This collection never says a domain
// genuinely lacks a property — a fact it does not carry is inapplicable (a
// stopped guest has no address because it is off) and rule 7 omits those. And
// the row publishes no name family at all: the reference's summary carries
...
const collectionDomains = "domains"
	...
	for _, collection := range order {
		if collection != collectionDomains {
			...
			out.emit(declineRecord{
				...
				Reason:     "unsupported",
				Detail:     "this collector serves domains only",
			})
			continue
		}
		if code := collectDomains(out, stderr, src, collection, generations[collection], &objects); code != exitOK {
			return code
		}
	}
