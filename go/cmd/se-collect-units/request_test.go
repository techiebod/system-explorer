...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect units\n",
		"token with an empty gen":       "collect units:\n",
		"token with a non-integer gen":  "collect units:abc\n",
		"token with a signed gen":       "collect units:-1\n",
		"token with a plus-signed gen":  "collect units:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Units:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect units:1 units:2\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect units:3\ncollect units:4\n", env)
	if code != exitOK {
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageUnits(t, healthyCapture())}
	code, stdout, stderr := runWith(t, "collect timers:11 units:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "timers" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for timers, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "timers" {
