...
// collector is judged on: a number's TOKEN. `Tls` is a boolean and `Priority`
// is an integer, and `true` is not `1` and `9223372036854775806` is not
// `9223372036854775806.0` under the harness's typed equality — DESIGN's
// "pass-through numbers keep the wire's type, not the language's". The
// committed capture makes that concrete rather than theoretical: Traefik gives
// `api@internal` priority 9223372036854775806, two below 2^63, which a float64
// round trip returns as 2^63 exactly. So a number is carried as its literal
// text and re-emitted from the same bytes it was read from, and nothing that
// leaves this collector is routed through a float64.
//
...
func decodeDocument(raw []byte) (*value, error) {
	...
	// in the payload is a broken capture, never a longer router list.
...
func decodeValue(decoder *json.Decoder) (*value, error) {
	...
	case json.Delim:
		...
		case '[':
			...
			// difference between [] and absent is a statement — a service with
			// `"usedBy": []` said something a service with no usedBy member did
			// not.
...
// the reference actually applies: `if raw.get("middlewares"):`, `raw.get(
// "entryPoints") or []`, `server.get("url") or server.get("address")`. Missing,
// null, false, zero, the empty string, the empty list and the empty object are
// all falsy there, and reproducing that exactly is what stops an empty
// `usedBy: []` from becoming a fact here and an absence there.
//
...
// countValue is a figure this collector COMPUTED, so it is rendered here
// rather than passed through: a base-ten integer with no exponent and no
// fraction, which is the only spelling `len()` has in the reference.
func countValue(n int) *value {
...
func (v *value) appendTo(dst []byte) []byte {
	...
	case jsonNumber:
		// The token as it was read: the priority that must not become 9.22e18
