// Command se-collect-traefik is the ingress-tier collector. It reads one
...
// terminal a person does. Those are the same contract, which is what makes this
// binary testable with a pipe and judged by the same harness that judges the
// Python reference it was ported from.
//
// It is the first port that reads an application over HTTP rather than a native
// host interface, so the endpoint is deployment configuration (SE_TRAEFIK_URL)
// rather than a path this file could know — see source.go, where that decides
// the absent reading.
package main
