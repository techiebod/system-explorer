...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect routers\n",
		"token with an empty gen":       "collect routers:\n",
		"token with a non-integer gen":  "collect routers:abc\n",
		"token with a signed gen":       "collect routers:-1\n",
		"token with a plus-signed gen":  "collect routers:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Routers:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect routers:1 routers:2\n",
		...
		// unknown rather than answered emptily: phase 3 owes object, evidence
		// and lookup, and a collector that answered them with nothing would be
		// serving a contract it does not implement (appendix C).
		"the object verb":   "object services api@internal\n",
		"the evidence verb": "evidence services api@internal\n",
		"the lookup verb":   "lookup route /api\n",
	}
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect routers:3\ncollect routers:4\n", env)
	if code != exitOK {
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	code, stdout, stderr := runWith(t, "collect middlewares:11 routers:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "middlewares" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for middlewares, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "middlewares" {
...
func TestTheCollectionOrderIsTheRequestLines(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	_, stdout, _ := runWith(t, "collect services:1 overview:2 routers:3\n", env)
	var order []string
	for _, commit := range ofKind(parseRecords(t, stdout), "commit") {
		order = append(order, commit["collection"].(string))
	}
	want := []string{"services", "overview", "routers"}
	for i := range want {
