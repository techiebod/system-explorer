// Command se-collect-packages is the installed-inventory collector. It reads
// one request line on standard input and writes to standard output; under
// socket activation systemd supplies both from the accepted connection, and in
// a terminal a person does. Those are the same contract, which is what makes
// this binary testable with a pipe and judged by the same harness that judges
// the Python reference it was ported from.
package main
