...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect packages\n",
		"token with an empty gen":       "collect packages:\n",
		"token with a non-integer gen":  "collect packages:abc\n",
		"token with a signed gen":       "collect packages:-1\n",
		"token with a plus-signed gen":  "collect packages:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Packages:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect packages:1 packages:2\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect packages:3\ncollect packages:4\n",
...
// A collection this collector never published is declined, never guessed at
// and never crashed on — and unsupported does not commit, so whatever a
// previous batch published for that name stands (DESIGN 18, 19).
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	dir := stageDpkg(t, healthyRows)
	code, stdout, stderr := runWith(t, "collect generations:11 packages:12\n",
		map[string]string{"SE_REPLAY_DIR": dir})
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	...
	if len(declines) != 1 || declines[0]["collection"] != "generations" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for generations, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "generations" {
