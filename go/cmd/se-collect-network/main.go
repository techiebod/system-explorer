// Command se-collect-network is the nftables collector: two collections,
// nft-chains and nft-rules, both derived from one native document,
// `nft -j list ruleset`. It reads one request line on standard input and
// writes to standard output; under socket activation systemd supplies both
// from the accepted connection, and in a terminal a person does. Those are
// the same contract, which is what makes this binary testable with a pipe and
// judged by the same harness that judges the Python reference.
package main
...
// (DESIGN 19), and this collector's payload is a firewall whose chain names
// and rule comments are whatever an operator pasted into them.
func run(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "object", "evidence":
		...
		// reassembled if whitespace splits it further; nft names and rule
		// handles carry no spaces.
		if len(fields) != 3 {
