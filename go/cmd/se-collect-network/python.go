...
// The reference builds two of this collector's values as PYTHON TEXT, not as
// JSON: Residue is json.dumps(statement) run through the envelope's bounding
// and scrubbing, and Rendered interpolates raw document values through
// f-strings. Both land in string facts that the harness compares character
// for character, so Python's spellings are part of the answer — its ", "/": "
// separators, its ensure_ascii escaping, its str(True) == "True", its
// lowercase hex(), its "None" for a missing member. This file is that
// vocabulary, kept together so the reasons live in one place rather than
// scattered through the two derivations.

// ── json.dumps, with Python's defaults ──────────────────────────────────

// pythonDumps is json.dumps(value) exactly: separators (", ", ": "),
// ensure_ascii, and the document's own member order — sort_keys is off, so a
// re-sorted residue statement is a different string from the one the
// reference emits.
func pythonDumps(v jsonValue) string {
	var out []byte
	return string(appendPythonDumps(out, v))
}

func appendPythonDumps(out []byte, v jsonValue) []byte {
	switch v.kind {
	case jsonNull:
	...
	case jsonNumber:
		return append(out, pythonNumberText(v.number)...)
	case jsonString:
	...
	case jsonArray:
		...
		for i, element := range v.array {
			...
			out = appendPythonDumps(out, element)
		}
		return append(out, ']')
	case jsonObject:
		out = append(out, '{')
		for i, m := range v.members {
			...
			out = append(out, ": "...)
			out = appendPythonDumps(out, m.value)
		}
		return append(out, '}')
	}
	return append(out, "null"...)
...
// appendQuoted is json.dumps' ensure_ascii string encoder: everything outside
// printable ASCII becomes \uXXXX (a surrogate pair above the BMP), which is
// why every residue string is pure ASCII by the time the bounding rule counts
// its characters.
...
// pythonStr is str(value) over a decoded JSON document. It is what every
// f-string in the renderer interpolates, so a missing member becomes the
// four-character word None and a JSON true becomes True — neither is a
// mistake to be tidied up, they are the reference's answer and the corpus
// holds them.
...
// printable. Reachable only where a document puts a container or a quoted
// string where nft puts a scalar, which no committed payload does — written
// out rather than approximated so that shape reads the same on both sides.
...
// exactly as written. Only the residue text depends on this; the facts carry
// the literal itself, because the harness compares parsed values there.
...
// pythonHex is hex(int): lowercase, unpadded, with the sign outside the 0x —
// hex(-16) is "-0x10". A mask is written and read in hex, which is why the
// renderer spells it that way; big.Int rather than int64 because Python's
// integers do not overflow and a mask read out of a document need not fit.
func pythonHex(v jsonValue) string {
	if v.kind == jsonBool {
		if v.boolean {
			return "0x1"
		}
	...
	n, ok := new(big.Int).SetString(v.number, 10)
	if !ok {
		return "0x0"
	}
	if n.Sign() < 0 {
...
// maxReasonLength is system_explorer.text.MAX_LENGTH, spelled at the residue
// call site in the reference as the literal 400.
const maxReasonLength = 400

// oneLine collapses whitespace and bounds on a word boundary. Every input
// here is the output of an ensure_ascii dump, so the only whitespace it can
// contain is the ASCII space and character counting equals byte counting —
// but the rune conversion is kept because the rule is stated in characters
// and a future caller need not be ASCII.
...
// variables, read from os.environ at call time, which would make this
// collector's output depend on the environment of whoever ran it. A replayed
// collector whose facts move with the shell that spawned it is not
// reproducible, and the substitution is aimed at exception text from an HTTP
// client rather than at a structured ruleset, which carries no such value.
// The two rules kept are functions of the payload alone.
func scrub(value string) string {
	return stripUserinfo(stripQueryStrings(value))
}

// A '?' anywhere in a serialised statement takes everything after it up to
// the next whitespace or quote — the reference's rule is a bare regex over
// failure text, not a URL parse, so it fires on any question mark the
...
// reasonText is envelope.reason(text, 400) — bound first, scrub second, in
// that order, because bounding a scrubbed string and scrubbing a bounded one
