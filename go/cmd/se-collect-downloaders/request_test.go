...
func TestMalformedRequestsExitTwoWithAStderrLine(t *testing.T) {
	cases := map[string]string{
		...
		"token without a generation":    "collect clients\n",
		"token with an empty gen":       "collect clients:\n",
		"token with a non-integer gen":  "collect clients:abc\n",
		"token with a signed gen":       "collect clients:-1\n",
		"token with a plus-signed gen":  "collect clients:+5\n",
		"token with only a generation":  "collect :5\n",
		"an unecho-able uppercase name": "collect Clients:5\n",
		"a colon-bearing name":          "collect a:b:5\n",
		"one collection, two gens":      "collect clients:1 clients:2\n",
...
func TestOnlyTheFirstLineIsARequest(t *testing.T) {
	...
	code, stdout, stderr := runWith(t, "collect clients:3\ncollect clients:4\n", env)
	if code != exitOK {
...
func TestAnUnservedCollectionIsDeclinedUnsupportedAndNotCommitted(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	code, stdout, stderr := runWith(t, "collect history:11 clients:12\n", env)
	if code != exitOK {
		t.Fatalf("exit %d, stderr: %s", code, stderr)
	}
	records := parseRecords(t, stdout)
	declines := ofKind(records, "decline")
	if len(declines) != 1 || declines[0]["collection"] != "history" || declines[0]["reason"] != "unsupported" {
		t.Fatalf("expected one unsupported decline for history, got %v", declines)
	}
	for _, commit := range ofKind(records, "commit") {
		if commit["collection"] == "history" {
			t.Fatal("an unsupported decline established nothing and must not commit")
		}
	}
}

// The request line's order is the emission order, and `at` advances across the
// whole batch rather than restarting per collection — one counter, because the
// judge's non-decreasing rule is scoped to a collection and a reader comparing
// two rows across collections should still see which was read first.
func TestTheRequestLineDecidesTheCollectionOrder(t *testing.T) {
	env := map[string]string{"SE_REPLAY_DIR": stageHealthy(t)}
	_, stdout, _ := runWith(t, "collect transfers:2 clients:1\n", env)
	var order []string
	for _, record := range ofKind(parseRecords(t, stdout), "commit") {
		order = append(order, record["collection"].(string))
	}
	if len(order) != 2 || order[0] != collectionTransfers || order[1] != collectionClients {
		t.Fatalf("commits followed %v, and the request asked for transfers first", order)
	}
}
