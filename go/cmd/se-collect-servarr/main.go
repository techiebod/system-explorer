// Command se-collect-servarr is the servarr apps, health, queue and history
...
// makes is a GET against a configured instance's REST API, authenticated by a
// key that rides the X-Api-Key header and appears in no URL, no fact, no
// decline detail and nothing this process writes to stderr.
package main
...
func run(stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "object", "evidence":
		...
		// reassembled if whitespace splits it further; the fan-out names
		// this collector publishes carry slashes, never spaces.
		if len(fields) != 3 {
