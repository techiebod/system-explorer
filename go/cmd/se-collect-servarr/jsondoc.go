...
// collector is judged on: a number's TOKEN. `QueueTotal` and `SizeLeftBytes`
// are integers the app stated and this collector passes through untouched, and
// `0` is not `0.0` under the harness's typed equality — DESIGN's "pass-through
// numbers keep the wire's type, not the language's". A stalled download's
// sizeleft is a byte count that can exceed 2^53, so a float64 round trip would
// lose digits off exactly the figure somebody is watching. So a number is
// carried as its literal text and re-emitted from the same bytes it was read
// from, and nothing that leaves this collector is routed through a float64.
//
...
// — the shape the reference uses for every nested read, most sharply on a
// history record's quality, where the name sits two objects down.
...
// exactly, and a fractional one truncated toward zero. The int64 path is first
// so a figure above 2^53 keeps every digit rather than arriving through a
// float. Only the queue walk needs it — totalRecords decides whether there is
// another page — and every other number this collector touches is passed
// through as the token the app spelled.
...
//
// Here it is also the port's one lawful divergence from the reference:
// health_item_facts and queue_record_facts write Source, Type, Message, Title,
// Status, TrackedDownloadStatus and TrackedDownloadState UNCONDITIONALLY, so an
// app that omits one publishes a null fact, which the contract's recursive
// fact_value and the replay judge both refuse. The reference cannot be
// reproduced faithfully and be lawful at the same time there, so this port
// takes the lawful reading and the divergence is reported rather than hidden.
