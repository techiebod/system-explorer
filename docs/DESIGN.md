...
- **A plugin surface, and what it is still missing** (§18/§06) — **RULED 2026-08-20: a collector this repository does not ship is a first-class collector, and the estate owns the ones that read estate concepts.** The closure delta is the first: the estate is what deploys, it already produces two of the delta's four answers, and a public product computing an estate's deployment diff would be carrying somebody's workflow as a first-party fact. So system-explorer owes the SURFACE and never the plugin. **Most of that surface already exists, by construction rather than by luck.** A collector is a binary that answers `declare`, `probe` and `collect` on a unix socket, registered as `name=/path/to.sock` in `SE_COLLECTORS`; the collator reaches it exactly as it reaches a first-party one and cannot tell them apart, which is the property the contract was written to have. The declaration travels with the binary, so a plugin brings its own facts, sentences, temperaments, prefixes and reference commands without this repository knowing they exist. **The gap is OPINIONS, and it is the whole of the gap.** `se.declaration/1` has no member for a rule: a plugin can publish a fact and cannot say what makes it alarming, so the estate's own delta could publish a risk classification that nothing grades. Opinions do not exist in the new stack yet either — the collator's ladders are unbuilt and the fixture driver still refuses an opinion expectation as unjudgeable — which makes this the cheapest moment there will ever be to decide it, and the reason it is recorded now rather than when the first plugin wants one. Two shapes are available and neither is obviously right: a declared rule table, which keeps opinions data and reviewable but bounds what a rule can say; or a plugin-supplied evaluator, which is unbounded and puts third-party code in the judging path. **A second question rides with it**, unanswered: the nix collector already publishes `ReceiptsExpected` and `Deployment`, read from a receipts directory that is an estate deployment workflow rather than a property of NixOS. By the same argument that moves the delta out, those may belong to the plugin too. `se-generation.json` is defensibly first-party — it is deliberately named for this agent rather than any estate and its shape is documented here — but the receipts beside it are not, and `corpus/nix/attested` was staged against them on 2026-08-19.
