...
> **Which half of completeness the manifest actually makes falsifiable, measured 2026-08-20**
>
> The manifest makes the **stream's** completeness checkable: the terminal's count against the manifest's entries, each state record's generation against the one the manifest claims, and each promised object count against what arrived. A receiver can refuse all three.
>
> **The manifest's own completeness it cannot check, and no receiver ever can** — the manifest is the only account of what the sender declares, so a manifest missing a collection and a complete one are the same document to whoever receives it. Found by reverting the emitter to list only the collections that send state: every wire-side guard stayed green and only the sender's own test, the one holding the manifest against the store, went red.
>
> So the rule lives where the answer is — beside the store — and this is stated rather than left implied, because a guard whose edge is unwritten gets trusted for the half it cannot see.
...
>
> Names in this product legitimately contain slashes, colons and spaces — `dataset:tank/photos` is an ordinary id. So the request line is **a fixed token count, length-bounded, with a declared encoding for anything containing whitespace** — the name token travels with each space as `%20` and each literal percent as `%25`, decoded in that order on arrival, exactly those two sequences and nothing else (not URL decoding); declared at R3d, when the fleet rollout reached the first collections whose names carry spaces (an nft table's `inet filter`, a socket's `tcp 0.0.0.0:22`, a mount point under /media), and implemented by every collector so the collator encodes for one fleet rather than a roster — and **a collector treats every token as data — never as a path fragment, never as an option, never as part of a command string.**
>
...
> **`plugins` is intent this repository does not own, and the shape it has is the point**
>
> Since appendix C's ruling of 2026-08-20 the protection collections belong to an estate rather than to this repository, and until phase 4 `se.intent/1` named `protection` as a member and closed the document against any other — one estate's concern spelled into the public contract, and every other estate's hub-tier plugin with nowhere to write its intent at all. That is the subset guard wearing a schema.
>
> **The way out landed with intent, in phase 4**, and protection moved into it as the worked example above. `plugins` is keyed by plugin name and each stanza is an object, because a plugin usually has more than one kind of intent to state — protection has targets and destinations — and an array would force the first to be the only one.
>
> **What the contract checks here, and what it cannot.** It checks the container: an object, keys shaped like a collector name, values that are objects. It cannot check a stanza's contents, and must not try — the moment this file validated protection's targets, the estate's concern would be back in the public contract by another route. So the shape of a stanza is the plugin's declaration to state, the hub asks that declaration, and **a misspelt stanza is indistinguishable from a well-formed one to this file alone**. Said plainly because a schema that looks total and is partial is trusted for the half it does not cover.
...
> **Who ruled what, because the distinction is worth keeping.** Most entries here were ruled by the owner. Two — `se.views/1` and the UI design system — were ruled on 2026-08-20 by the agent executing phase 5, because PLAN makes the second a precondition for that phase starting and both proposals were already written, conservative, and continuations of what ships today. Each names what would reopen it. **An owner override costs nothing and is expected**: neither has code built on it beyond the phase that began the same day.
...
- **`se.views/1` survives unchanged** (§29) — **RULED 2026-08-20, on the written proposal and without amendment.** The schema, the deployed-directory pattern and the documents an estate already carries port as they are. A view is operator judgement about ARRANGEMENT and a problem domain is an answer to a question; neither replaces the other, and when problem domains mature a view may become one way of rendering them. Deleting a shipped, working surface to force that question early would be churn without a consumer. **What would reopen it:** a problem-domain page that cannot be expressed as a view, or a view that can only be written by restating a fact the declaration already carries — the second being the fourth-copy shape, and the one to watch for.
- **Disclosure is declared per value as `discloses`** (§21) — settled; the vocabulary and the well-known table are in appendix A.
- **The UI design system is the production token set, no external library** (§06) — **RULED 2026-08-20, on the written proposal and without amendment.** The palette, type scale, spacing and chips the current UI ships are carried forward as *the* library. It is small, it is owned, and both scales already speak it — and the phase that would adopt a framework is the same phase that has to render two scales from one token set, so importing somebody else's vocabulary is the moment the product stops owning its own. **The constraint this puts on plugins is the one appendix C's plugin entry names**: a plugin expressing itself in states and severities the product already defines gets no UI code, and a plugin introducing a new one moves a lint-enforced literal in this repository. Both halves of that are gate 5's.
- **Partial verb service at the skeleton** (§18) — ruled provisionally: the system collector serves declare/probe/collect and exits 2 on the other three declared verbs; phase 3 owes object, evidence and lookup, and the acquisition-time evidence commitment arrives with the evidence verb rather than being faked without it. **DELIVERED IN PART at R3c (2026-08-21), provisionally under the re-baseline's register assignment (rows 1–2):** the `object` and `evidence` verbs land on the champions, with the contract members §19 promised and never grew — a per-collection `verbs` bounds table in the declaration, and two response records in the stream contract: an object response is NDJSON (one full-density object record, its unobservable and relation_assertion records, a `verb_end` terminator whose `truncated` member states a bound that bit), and an evidence response is one `evidence_document` header line — media type, digest under the collect-time regime, byte count — followed by exactly that many raw payload bytes and the same terminator. A name the collector never published is a decline record then the terminator, because a decline is data under every verb. **DELIVERED IN FULL at R3d (2026-08-23):** `lookup` landed on storage (`snapshots-of`) and network (`route-get`, `resolve`), with two rulings the R3c text left open. The palette is declared at the **declaration root** — an optional `lookups` member mapping each name to its question, input description, example and bounds — not per collection, because a lookup belongs to the collector and cuts across its collections: snapshots-of answers about datasets, route-get about links. And the response is **one object record in the `lookups` namespace** — name `<palette>/<input>`, type `lookup-result` — plus relation_assertions where the answer joins a published row, then the same terminator; a tool's refusal that IS an answer (the kernel's no-route, resolved's NXDOMAIN) lands in the facts as data, never as an exit. One stated deviation rides the network declaration: busctl surfaces resolved's failure message and not the D-Bus error name, so `Result` carries the daemon's words where the old adapter carried a verdict constant. **The rows-1–2 rollout the R3c text left to R3d landed with the same wave (2026-08-23): all twenty collectors serve `object` and `evidence`**, each answering with the same records its collection stream publishes and each evidence payload in its reference adapter's own shape, with the redactions ported where the reference redacts (traefik's backend userinfo, servarr's history URLs, docker's assignment lists, paperless's connection URLs, the systemd manager's Environment). The request line's whitespace encoding, promised above as "a declared encoding", became concrete here — %20/%25 on the name token, every collector decoding — when the rollout reached the first names that carry spaces.
...
- **A plugin defines four layers, not one** (§06/§18/§28) — **RULED 2026-08-20: a plugin is not a collector that is allowed to have opinions.** It defines its collector's observations, the identities it asserts, the hub-level findings it mints, and how it is represented; the tier question this was raised as is one sub-case of that. Each layer has a real attachment point and each was checked rather than assumed. *Observations* attach at `declare`/`probe`/`collect` and the declaration that travels with the binary. *Identities* attach at `object_prefixes()` in `agent/envelope.py`, published through `/v1/capabilities`, so a cross-reference to a plugin's object resolves to its own page rather than to another collection's. *Findings* attach at a rulebook module and — where the verdicts are estate-scoped — a hub view. *Representation* needs nothing at all, **if** the plugin speaks the vocabulary the product already has.
  ...
  **RULED 2026-08-20, and the rewrite answers it structurally rather than by choosing between the two ways out.** *Severity* is bound by the contract: a rule's `level` `$ref`s the closed vocabulary, so a plugin **cannot** introduce a severity the product does not define — which is the "bind plugins to the existing vocabulary" option, enforced by the schema rather than held by convention. *State words* need no binding at all, because the new stack's renderer is the producer: a server-rendered page switches on nothing, so a plugin's own vocabulary renders the day it arrives and an unknown member renders unstyled rather than erroring. What is NOT carried forward is the shape that made this a problem — `app.js`'s lint-enforced copy of a vocabulary no endpoint publishes is the old stack's, and it is disposed of at the cut with the rest of it. The paragraph below describes that stack and stays as the diagnosis it is.

  **That last condition is where the architecture currently forbids what a plugin needs — in the SHIPPING stack.** `ui/app.js` keeps its own literal copy of the severity ordering and the conformance suite lints that copy against the rulebook's tuple; `conformance/test_rules.py` states the reason in place — *"No endpoint publishes the level enum, so app.js is forced to keep its own copy."* `VALUE_CLASS`, the state-badge table, has the same shape. So a plugin introducing a state word or a severity this product does not already know cannot be data-only: it has to move a lint-enforced literal in this repository's UI. Two ways out, offered as a pair rather than as a choice — **publish the enums**, so `app.js` stops keeping a source copy and the lint guarding it is deleted with it; or **bind plugins to the vocabulary that exists**, which is probably the right default and today holds only because nobody has tried. Gate 5 owes whichever, surfaces being gate 5's.
...
- **An unknown declaration hash cannot be fetched once the connection reverses** (§06/§18/§22) — **RULED 2026-08-20 while building the hub's session, because there is only one available reading.** The checkpoint contract says the hub matches a manifest's declaration hashes against its held copies and that "an unknown one is fetched rather than guessed". That sentence predates the reversal: a hub has no way to reach a collator, by construction, and that unreachability is the property the reversal exists to provide — so there is no channel a fetch could travel on. It is not a gap to fill either, because the order already prevents it: **declarations travel first, precisely so nothing needs fetching.** A manifest naming a hash the hub does not hold is therefore a collator that skipped a step, and the hub refuses the checkpoint with the hash named rather than promoting state against axes nobody holds. Recorded rather than silently reinterpreted, because the contract's own description still carries the older sentence and a reader would otherwise take the refusal for an unimplemented fetch.
- **A never-applied collection has no honest freshness** (§06/§19) — open, measured 2026-08-20 while running a real collator against a real hub. A manifest entry carries `generation` and `freshness`, and `freshness` is required. A collection whose generation was issued and whose first collect was then REFUSED — a structural violation rather than a decline — has generation 0 and no decline reason, so the emitter can only say `current`: the schema's `stale` requires one of the four decline reasons and a rejection is not one of them. **`generation: 0, freshness: current` is a contradiction a hub can read as health.** Generation 0 already says *never applied*, so the pairing is vacuous rather than false — but a hub that renders `freshness` without reading `generation` beside it shows a collection nobody has ever read as current, which is this product's own absence-as-health failure inside its own manifest. Three ways out and none is obviously right: make `freshness` optional at generation 0 (a missing member is a third state to handle); widen `stale_reason` to carry a rejection (it is currently exactly the decline vocabulary, and widening it makes the four-reason authority table stop being closed); or state that generation 0 dominates and require every consumer to check it first (free, and the kind of rule that holds until somebody writes the second consumer). Not ruled while phase 4 is landing, because it is a contract change and the shape it wants may be clearer once a surface renders a manifest.
- **An object's type joins the stream record** (§19, PLAN R1) — **RULED 2026-08-21, owner-approved.** The stream's `object` record gains an optional `type`, and the checkpoint's object shape follows. It is structural observation — `scsi-host`/`expander`/`disk`, `service`/`slice`/`device` — and it decides which health statement a row is entitled to, which column set reads sensibly, what a facet bar can offer and what a hide rule may match. It is not derivable from anything else on the wire, and the port's own source recorded the loss when it was made: *"the TYPE never reaches the wire… but it decides which health statement a row is entitled to."* Optional because homogeneous collections (packages, journal) genuinely have none; a heterogeneous collection omitting it is a port defect the inverted comparator now sees.
- **Applied order is preserved, and trees derive from relations** (§13/§19, PLAN R1) — **RULED 2026-08-21, owner-approved, and it corrects a first proposal.** The store discards emission order today (objects served sorted by id) while both implementations of `units` record that the systemctl-status order *is* the collection. So the store keeps the applied sequence and serves it, through REST and checkpoint alike. Hierarchy is deliberately NOT a `depth` member on the wire: depth is *caused by* a relationship, so the producer asserts `member-of`/`enslaved-to` edges and the collator — the tier that holds every row — derives the tree. A shipped integer would be a second copy of what the relations state, disagreeing with them the first time an edge moved. Sibling order within a parent is the preserved emission order, which is where a collection's argued ordering (most diagnostic first, the device wall last) lives.
