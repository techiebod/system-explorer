...
7. **The live comparator is named as a venue five times and does not exist.** It is where DESIGN 19 sends all three replay bounds — a constant-but-plausible `boot_id`, a constant `at`, self-reported cost — and where both named residuals send their truth; it is also gate 3's parity report. Until it is written, five deferrals point at a promise, and a reader of the ledger would take them for coverage. The residual entries now say so in their own text. Building it is the last piece of this phase: the old agent and the ported collector on one guest, observing one machine, their answers diffed per collection.
