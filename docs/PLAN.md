...
| 6 | name families on `units`/`hardware` (and fleet-wide audit) | five collections carry a family; the R3d audit names three whose stable identity is still only a fact — block-devices, arrays, tailscale — each owed as its own landing (register row 6's prose has the reasoning) | R3c/R3d |
