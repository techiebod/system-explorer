...
> · **a two-guest estate renders one coherent view with reach and coverage stated** — **NOT JUDGED, and not claimed.** What is proven, at the CI tier and by the real binaries: two `se-collate` processes dial one live hub listener over sockets, both promote, the estate coheres into one view with reach and coverage stated, the founding question is answered over it, and a second hub federates against the same intent hash — answering for its own site and refusing a third by name. What that does NOT prove is the clause as written, which standing rule 3 puts on **two lab guests as two sites**: a real network between the parts, two kernels, and the NAT-mode dial against a host that can only originate.
>
> The lab could not be run at all. `virsh` is absent on the machine this work was done on, and the lab's remote mode wants a KVM host — which in this estate means a real machine under a read-only standing constraint, so bringing guests up there is the owner's decision and not this agent's. **That is a probe that could not run, not a test that passed**, and the two must not render the same. Gate 4 therefore stays shut on one clause, which is the whole of what it is waiting for.
