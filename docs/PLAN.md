...
| 8 | rule tables fleet-wide (restart-churn, SMART verdicts, link-rate…) | thirty-one rules across the two champions at R3c; the fleet's expressible tables landed at R3d (sixty-seven more rules over thirteen collectors, each fired in its own tests), with the inexpressible residue stated per table and owed to derived facts — docker's landed first (Health and ExitCode minted on both implementations, four rules fired) | R3c/R3d |
