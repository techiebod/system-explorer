...
| 7 | relations on `units`/`hardware` (and fleet-wide audit) | declared by both champions at R3c; the R3d audit fixed the journal verb's undeclared member-of (a latent collator rejection) and names three reference object-level edges still unported — docker plumbed-onto, vms owns, units runs (register row 7's prose) | R3c/R3d |
