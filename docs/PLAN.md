...
| 6 | name families on `units`/`hardware` (and fleet-wide audit) | eight collections carry a family: the audit's three landed on both implementations the day they were named — block-devices' by-id and partuuid spellings, arrays' md uuid, tailscale's public key — judged equal by the corpus replay | R3c/R3d |
| 7 | relations on `units`/`hardware` (and fleet-wide audit) | declared by both champions at R3c; the R3d audit found the undeclared-palette defect twice (journal, then units itself) and both are fixed with pins; the reference's opened-object edges landed — docker's container and network edges, vms' owns, units' runs — leaving only vms' bridge and PCI edges, which await the facts they would cite | R3c/R3d |
| 8 | rule tables fleet-wide (restart-churn, SMART verdicts, link-rate…) | thirty-one rules across the two champions at R3c; the fleet's expressible tables landed at R3d (sixty-seven more rules over thirteen collectors, each fired in its own tests), with the inexpressible residue stated per table and owed to derived facts — docker's, downloaders', paperless's and nix's landed (Health, ExitCode, DiskUsedPercent, StorageUsedPercent, DeploymentOutcome minted on both implementations); the residue is now only the two structural mints — resources' slices and protection's targets — whose derivation IS the rule | R3c/R3d |
...
| 17 | change tracking — history, `/v1/changes`, `what_changed` | landed at R3e, at the COLLATOR — §06 gives the record to the tier that holds both samples, and this row's own probe had been pointed at the hub. Measures excluded by declaration rather than by a list; a snapshot written only when the diffable content moved; a question before the record begins answered as a stated gap | R3e |
...
| 19 | acknowledgement — appended/attributed/reversible, write posture | landed at R3e: an append-only transition log whose state is folded rather than stored, on a write listener of its own that establishes the actor from a credential and refuses reads — so the read surface stays structurally read-only. Resolution is not a transition it accepts | R3e |
...
| 33 | two collections declare the prefix `unit`, and it cost the host every fact | fixed 2026-08-23 at the blast radius: the kind resolves to nothing, the host keeps its facts. The clash itself is a producer change and still stands | R4 |
...
- **R3c — structure retrofit on the champions** — **done 2026-08-21, except `lookup` (register row 3, R3d's)**: `units` and `hardware` carry all six layers and serve `object` and `evidence`. units asserts `member-of` per parented row so the slice tree derives from edges on both implementations, declares the five-rule table (unit-health at `interface`, restart-churn at `threshold`, the absent-dependency pair and their unobservable sibling), and its object verb serves the eleven density facts the row deliberately cannot afford — NRestarts among them, so restart-churn fires where the density is in hand, as the old detail-only rule did. hardware publishes the identity chain as name families, asserts `attached-to`/`backs`/`member-of`, and declares twenty-six rules carrying the recorded lessons: SMART's no-reading-is-a-fact discipline, the link-rate opinion's info-versus-warn split, and the stale-snapshot three-way. Four derived facts are minted identically on both implementations where the closed condition vocabulary cannot express a comparison between two facts or a substring test — and must not grow to.
  ...
  **Exercised live on the lab guest, not only in replay**, because a corpus proves a port reproduces a capture and nothing about the machine: both champions' verbs answered on the running guest — the disk object carrying every name and every declared fact, the evidence document beside it holding the thirty-two raw sysfs attributes with a digest that checks against the bytes as served, and the three environment members absent from a unit's 359 published properties — and one collator round over the hardware and storage collectors resolved every asserted edge, including the cross-collector `backs block-device:sda` this phase fixed. The zpool half of that chain is the storage collector's own `backed-by`, and the join between them is the fixture's.
- **R3d — the rest of the fleet** — **done 2026-08-23**: the six owed collections (nft-tables, port-exposure, tailscale, datasets, arrays, plex/requests) emptied NOT_YET_PORTED; the `lookup` verb landed with the palette-at-declaration-root and object-record-response rulings (storage's snapshots-of, network's route-get and resolve, with the busctl deviation stated in the declaration); `object` and `evidence` rolled out to all twenty collectors, each evidence payload in its reference adapter's own shape with the reference's redactions ported, and the request line's whitespace encoding (%20/%25 on the name token) declared and implemented fleet-wide when the rollout reached the first names that carry spaces. The expressible half of every reference rulebook is a declared rule table fired in its collector's own tests — sixty-seven rules over thirteen collectors on top of the champions' — with the inexpressible residue stated per table and owed to derived facts. The rows 6/7 fleet audits are recorded in the register's own coverage prose: three name families owed as their own landings (block-devices, arrays, tailscale), three reference object-level edges unported (docker plumbed-onto, vms owns, units runs), and the audit's one live defect — the journal verb's undeclared member-of, a latent collator rejection — fixed and pinned the day it was found.
- **R3e — the features** — **done 2026-08-23**: findings persistence, the views route and sibling reads landed first; the other three waited on owner rulings taken the same day and then landed behind them.

  **Change tracking landed at the COLLATOR, and the row's own probe was the first thing that had to change** — it had probed the hub since it was written, encoding an assumption §06 contradicts: the collator keeps the record, because it is the tier that already holds both samples. The migration question appendix B left open is ruled — tracking begins at the cut, nothing is translated forward, and a question reaching before the record begins is a stated gap rather than an answer from an empty baseline, because an unreachable baseline is not an empty one and diffing against nothing would report every object as added. The measures are excluded **by declaration rather than by a list**, which is the one thing this implementation can do that the reference could not: a fact whose declared temperament is counter or gauge reaches neither the diff nor the snapshot, so a plugin's counter is excluded the day it arrives and there is no table to fall out of step with one. A snapshot is written only when the diffable content moved, so a host whose counters advance every sweep stores one row rather than one per sweep.

  **Acknowledgement got a listener of its own**, which is the whole of the posture ruling: `http.py` holds read-only structurally — only GET is routed — and that is what the hub's bind rests on, so a write verb on the same socket repeals it whatever the route's own authorization does. The transition log is append-only and its state is folded rather than stored, so the log and the state cannot disagree; the actor is established from a credential and a body that supplies one is refused rather than overridden, because a self-declared actor is a claim and the two are indistinguishable once stored. Resolution is not in the action set: it is observed, and a route that let an operator declare it would be the one write that makes the product lie about the system rather than about its own noise.

  **The rule-table residue closed with a named-evaluator seam** rather than the derived facts R3d owed it to. A rule row may name an in-tree evaluator in place of a `when`, keeping every other member, so the opinion stays declared and only the test is code; the set is closed and a declaration NAMES an evaluator and cannot ship one, which is the line that keeps third-party code out of the judging path. Six evaluators, not the two the estimate said — the estimate counted rule families, and the slice states three things while protection's three are the branches of one `if/elif/else` each carrying its own exclusion. The correction is recorded because the reopening test is a count.
  ...
  **A fifth was the same sweep's other half, and this one was built.** `wire.Begin.Timens` was parsed and read by nothing, while all twenty collectors have reported it since the field existed and DESIGN §09 states that the collator "compares its own offset against the collector's at handshake and states a mismatch rather than correcting for it" — so the designed defence had one half of a handshake. It is the sharpest instance of the shape because of *why* nothing else catches it: `CLONE_NEWTIME` offsets `CLOCK_BOOTTIME` per namespace and **leaves the boot id identical**, so the cross-boot branch cannot fire and the age is wrong by exactly the offset while looking entirely ordinary. The one case caught before was a skew large enough to drive the age negative — the subset that is self-evident. A *forward* offset renders a stale collection **fresh**, which is the direction that matters. Built with the skew stated and the age withheld, and with zero kept distinct from never-compared, because agreement is a reading and is what lets a person trust an age. Unlike the fourth, no ruling was needed: the behaviour is specified and the surface member already existed.
...
> **GATE R3 OPEN, 2026-08-23. Both clauses hold, and the record below is the road to it rather than a summary of it.**
>
> **Clause 1 holds.** The whole-surface run on the Debian guest reads **20 of 20 clean, nothing unreachable, 957 objects on each side**. It is reproducible and the invocation matters, so `se-compare`'s own docstring states both conditions the answer depends on — privilege, and `/usr/sbin` on PATH — because neither is checkable from inside the tool and a run missing either reports agreement about questions neither side could ask.
>
> **Clause 2 holds:** 32 rows, every built row's probe passing, every owed row owned by a later gate.
>
> Kept below because the route is the evidence, and because three of these were found only by running the thing rather than by reading it.
>
> **First run, and clause 1 did not hold.** On a real systemd host it was **17 collectors at parity, 2 the reference cannot run there** (`network/port-exposure`, and `plex/requests` for want of a seerr) **and one differing: `system`.**
>
> **Re-measured the same day: 20 compared, 19 clean, nothing unreachable, and the only remaining difference is the four `system/identity` facts.** The `network` half was never a parity failure — it was a venue defect wearing one. Debian puts `nft` in /usr/sbin, which is not on a non-root user's PATH, and both implementations resolve their tools by PATH lookup. The reference raised FileNotFoundError and exited 2, losing **all nine** of its collections rather than the one that needed the tool, where the port declined the four affected collections and still published 48 objects. That is an adjudication in the port's favour on robustness, and it stands whatever the venue does. With `/usr/sbin` reachable and the run under `sudo` — the deployed agent holds capabilities the comparator's shell does not, and `nft list ruleset` refuses outright without them — both sides publish 68 objects and 7 relation assertions and agree exactly.
>
> The provisioning script's first attempt at that venue tested `command -v nft` and concluded "not installed" about a package already at its newest version, then ran an install that printed nothing and changed nothing. **A tool's presence and its reachability are different facts and only the second one is the venue** — which is the same error the comparator itself had been reporting, one layer down.
>
...
>
> **The last item was one collector and it was the owner's.** `system/identity` emits `Hostname`, `OsId`, `OsPrettyName` and `OsVersionId`, which the reference does not. The port emitting MORE is what §20 reserves for the owner, and the comparator could only express an accepted divergence in the other direction — so there was no way to record "the port wins" without the ruling.
>
> **Ruled 2026-08-23: the port wins.** The reference had os-release open and published none of it, so its identity object carried neither the host's name nor which OS it is. The facts are already read correctly, and dropping them to make a parity count come out even is the deletion-to-hit-a-number this repository has a standing rule against. Recorded as the first and only entry in `PORT_ONLY_DIVERGENCES`, stripped from the **port** before the diff and never from the reference — so a reference that starts emitting any of the four is itself reported, and a divergence on any fact outside the set still fails.
>
> The guard that had held that table empty could not simply be deleted when the ruling arrived: "the table is empty" expires the moment a ruling exists, and removing it would leave the table unguarded exactly when it first holds something. It became **every entry is a ruling** — carrying its own words, dated, and findable in this document — so an entry cannot be a convenience someone added to turn a red run green. Three plants: an entry with no ruling, a one-word ruling that is a label rather than a reason, and a fact this page never records.
>
...
**Built 2026-08-23 across six waves, and running on the Debian lab guest for the side-by-side.** The declaration-driven widget layer; the row → object → evidence drill, every step an `<a href>`; the four empty states separated where a person can see them; trees from the producer's own `member-of` edges with indentation disabled under sort; cross-subsystem links through the shared prefix index; the identity chain, which needed `store.ObjectRow` to start reading the `names` column it had written since the store existed; facets as links; the hide-group mechanism with its four invariants; the absent-severity mark; and `/v1/findings`, which had no route at either tier.

**Three things are recorded rather than done, and each for a stated reason.** Two rules of §28 are undeliverable from today's declarations — no declaration ships a counter's companion rate gauge, and twenty booleans declare no labels — so both are rendered honestly rather than approximated. No shipped declaration declares a hide group, because adding the member moves a declaration hash and every corpus pinning it, which is a producer change this repository deliberately makes deliberate. And the `units`/`workloads` prefix clash is fixed at its blast radius, not at its cause.

**Then the owner opened a page, and the gate earned its keep a second time.** Every assertion in `pages_test` checked that a page CONTAINED the right sentence; none checked that it did not also contain a contradictory one. So 18 of 52 collection pages shipped saying *"What follows is the last reading that did apply"* directly above a panel headed **Never read** — through 5,300 green assertions. An adversarial audit of the real rendered output then found 82 defects, 71 of which verified.

**They are one defect wearing fourteen faces, and it is this document's founding one.** The index chip *defaulted* to `current` and only downgraded, so a collection never read rendered as healthy. The opinion roll-up skipped generation-0 collections with a bare `continue` and then printed "No opinion fired on this host's own facts" over an estate a third of which had never been read. The estate index counted OBJECTS, so 29 of 52 collections per host were invisible from the hub — precisely the ones not answering. The estate served dark hosts' last-known counts with nothing marking them last-known. The opinions table sorted by host and never rendered it, so a critical could not be attributed to a machine. And the decline sentence asserted "an incident, not a configuration" directly above the collector's own "no servarr api configured" — contradicting a 2026-08-19 ruling, which is §27's forbidden move committed in prose, where it does not look like a severity table.

**The guard that was missing is the one that explains the rest**: coherence is a property of a page, not of a sentence, and presence-testing cannot see it. `TestNoCollectionPageContradictsItself` now walks all eleven reachable state combinations asserting mutual exclusion. The residue is row 36 and is not small.

**The gate found something no suite had.** Standing the surface up on the guest showed 52 collections issued and 0 objects stored: `units` and `workloads` both declare the prefix `unit`, `store.PrefixIndex` refuses a contested prefix correctly, and the batch apply returned that error — so a host running both collectors applied nothing at all, while every page read "never read" and every log line named a relation problem rather than the outage. No test caught it because the first round of a fresh store applies before the rival declaration is in it; the conformance guard now runs a second round, which is the whole of what makes it bite. After the fix: 34 applied, 955 objects, 28 declines split 18 unavailable / 10 absent. That is the argument for the gate being a person looking at a page.

**Then the owner used it, and said it was a step backwards.** They were right, and the three things they named were each real: every mount below the root linked to a 404 (an object name is not a URL path segment, and `esc()` does nothing to a slash); most relationships were invisible (the page read only OUTBOUND edges, and only within the object's own collection — a slice showed 1 edge where it had 182); and there was no navigation at all beyond a breadcrumb. A crawl of the live surface now follows 2,487 URLs with none broken, which is a claim no assertion here could make before, because nothing had ever followed a link.

**And a ruling was recorded that the owner never made.** §28 said "one tier cannot run script at all, and the other names the single script it runs" — assembled from an incidental CSP header value and the owner's actual words, which were *"to encourage more modern html/css usage over js which is hand-rolled"*. A preference about how to build, turned into an architectural ban, in a governing document, attributed to them. Three guards then enforced it, and each would have had to be DELETED to add the first filter — which is how a wrong belief becomes load-bearing. Corrected: the line that survives is §27's, binding every language equally — script may change what is shown and may never decide what a thing means — and the guards now assert that, reading the one script's own text against it.

**What the surface carries now**: a navigation rail on every page, grouped by the collector each collection was learned under and derived from the store's own join, never a hand-kept table; both directions of every relation, across collections, grouped so a state sentence is said once rather than 181 times; `LoadState` on the units row, so 19 units whose file is not installed stopped rendering as ordinary stopped services; three hide groups taking 194 of 304 rows off the default view while every one stays in the markup; an attention facet narrowing by the rulebook's own verdict; the typed filter, admitted by SHA-256; and 23 unit declarations so a mount reads `13% of 15.55 GiB` rather than `13 of 16694517760`.

**Two over-claims of this surface's own were found and fixed by the same discipline.** The clean mark said "every rule this collection declares was evaluated" 302 times on a page where `restart-churn` structurally cannot run, because it tests a fact the producer deliberately keeps off the row. And a page whose declaration describes NOT ONE fact its objects carry now says so, rather than reporting that the producer "stated no value" — three collection NAMES are claimed by two collectors each, and the host's headline page was asking another collector's questions of this one's answers.
