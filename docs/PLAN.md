...
>
> Clause 2 holds: 32 rows, every built row's probe passing, every owed row owned, and the two lists that used to sit outside the census joined to it.
>
...
>
> **Re-measured 2026-08-23 on the Debian guest: 20 compared, 19 clean, nothing unreachable, and the only remaining difference is the four `system/identity` facts.** The `network` half was never a parity failure — it was a venue defect wearing one. Debian puts `nft` in /usr/sbin, which is not on a non-root user's PATH, and both implementations resolve their tools by PATH lookup. The reference raised FileNotFoundError and exited 2, losing **all nine** of its collections rather than the one that needed the tool, where the port declined the four affected collections and still published 48 objects. That is an adjudication in the port's favour on robustness, and it stands whatever the venue does. With `/usr/sbin` reachable and the run under `sudo` — the deployed agent holds capabilities the comparator's shell does not, and `nft list ruleset` refuses outright without them — both sides publish 68 objects and 7 relation assertions and agree exactly.
>
