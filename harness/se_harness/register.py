...
REGISTER: tuple[Row, ...] = (
    ...
    Row(45, "the acquisition economy is declare-only",
        ...
        "FRESHNESS: **BUILT 2026-08-24, host tier** — freshnessVerdict "
        "(freshness.go) compares the oldest contributing read against 2x "
        "the declared promise (the grace is one extra promise-window, "
        "derived from the declaration, because the scheduler's own pace "
        "ages the tightest collection to its full promise routinely; the "
        "owner may prefer the strict reading — one line). Overdue and "
        "unverifiable are distinct states and neither wears the current "
        "chip; wired through the host chip, the rail, the collection "
        "banner, /v1/collections and /v1/status's new `overdue` list; "
        "proven on the wire per rule 8 (a 1s-promise reading served "
        "overdue over real HTTP from the real binary). STILL OWED here: "
        "the checkpoint cannot carry it (closed enums — row 41), so THE "
        "HUB STILL SEES CURRENT; and the REASON vocabulary (§15: budget/"
        "timeout/pressure) waits on per-attempt outcome recording. COST: "
